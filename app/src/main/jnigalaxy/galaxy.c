...
DIR * newopendir(const char * pathname){
...
int hookOpenDir(){
	if (registerInlineHook((uint32_t) opendir, (uint32_t) newopendir, (uint32_t **) &oldopendir) != ELE7EN_OK) {
		return -1;
	}
	if (inlineHook((uint32_t) opendir) != ELE7EN_OK) {
		return -1;
	}
	return 0;
}
int unHookOpenDir(){
	if (inlineUnHook((uint32_t) opendir) != ELE7EN_OK) {
		return -1;
	}
	return 0;
}
